...
from .utils.query_processor import process_resume_query
            ...
            if resume_id:
                ...
                if enhancement_suggestions:
                    try:
                        ...
                        questions_format = enhancement_suggestions
...
class SentenceEnhanceView(APIView):
    """
    Combined API endpoint for:
    - GET: Retrieve suggestion questions by resume ID
...
class ResumeQueryView(APIView):
    ...
    async def _async_post(self, request):
        try:
            # Check content type and extract data
            if request.content_type == 'application/json':
                json_data = request.data.get('details')
                user_query = request.data.get('query')
                job_description = request.data.get('job_description')
            else:
                json_data = json.loads(request.data.get('details'))
                user_query = request.data.get('query')
                job_description = request.data.get('job_description')
            
            # Check for job description in the resume data if not provided directly
            if not job_description and json_data and 'JD' in json_data:
                job_description = json_data.get('JD')
            
            # Validate required fields
            if not json_data:
                return Response(
                    {"error": "No resume data provided", "success": False},
                    status=status.HTTP_400_BAD_REQUEST
                )
                
            if not user_query:
                return Response(
                    {"error": "No query provided", "success": False},
                    status=status.HTTP_400_BAD_REQUEST
                )
            
            # Process the query
            result = await process_resume_query(
                json_data=json_data,
            ...
            return Response(
                {"error": "Invalid JSON data", "success": False},
                status=status.HTTP_400_BAD_REQUEST
            )
        except Exception as e:
            logger.error(f"Error processing resume query: {str(e)}")
            return Response(
                {"error": str(e), "success": False, "data": request.data.get('details')},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )