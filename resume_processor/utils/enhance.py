...
async def enhance_resume_by_sections(
    ...
    Only enhances sections defined in the selected template.
    """
    ...
    template_params = TEMPLATE_PROMPTS.get(template_type, TEMPLATE_PROMPTS["simple"])
    allowed_sections = [param["name"] for param in template_params.get("parameters", [])]
    ...
    for section_name, section_data in resume_data.items():
        ...
        if mapped_section in allowed_sections:
            ...
            logger.info(f"Skipping enhancement for section {section_name} - not defined in template {template_type}")
